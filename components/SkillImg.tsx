  ...
  const link = linksArr.map((imgLink) => (
    <li key={linksArr.length} className=" list-none relative">
      ...
      <motion.img
        ...
        src={imgLink.link}
        alt=""
        className="rounded-full border border-gray-500 object-cover w-14 h-14 sm:w-24  sm:h-24 md:w-28  md:h-28 xl:w-32 xl:h-32  transition duration-300 ease-in-out wrap"
      />
    </li>
  ));
